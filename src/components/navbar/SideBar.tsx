        ...
        <aside className='bg-preset sticky min-h-screen w-max'>
            <section className='flex flex-col justify-center mx-3 font-ubuntu'>
                ...
                <div className='grid grid-flow-row w-max gap-2 my-2 capitalize text-white text-lg '>
                    <Link
                        href={`/absensi`}
                        className={`rounded-xl py-1 px-2 ${path === '/absensi' ? 'bg-white text-preset ' : 'border text-white'}`}
                    >
                        Absensi
                    </Link>
                    <Link
                        href={`/data-pegawai`}
                        className={`rounded-xl py-1 px-2 ${path === '/data-pegawai' ? 'bg-white text-preset' : 'border text-white'}`}
                    >
                        Data Pegawai
                    </Link>
                    <Link
                        href={`/kalkulasi`}
                        className={`rounded-xl py-1 px-2 ${path === '/kalkulasi' ? 'bg-white text-preset' : 'border text-white'}`}
                    >
                        Kalkulasi Gaji
                    </Link>
                </div>
